...
An elegant, from-scratch implementation of the Llama 3 architecture engineered purely in PyTorch[cite: 1]. This engine completely bypasses high-level library abstractions to manually construct the foundational mathematical mechanics, multi-dimensional tensor routing, and autoregressive inference pipeline of modern frontier models from first principles[cite: 1].

### Key Architectural Highlights
* **Mathematical Purity:** Every layer—from tensor rotation in the complex plane to root-mean-square normalization—is derived and written manually[cite: 1].
* **Weight Realignment:** Features a custom loading script designed to extract official open-source weights (e.g., Llama 3.2 3B) and dynamically map parameter dictionary keys into this custom scratch execution engine[cite: 1].
* **O(1) Inference Scaling:** Implements a production-grade static KV-cache mechanism to achieve constant-time complexity per token generation, maximizing memory bandwidth efficiency[cite: 1].
...
To resolve the massive memory bandwidth bottleneck imposed by the Key-Value (KV) cache during autoregressive decoding in standard Multi-Head Attention (MHA), this engine implements Grouped-Query Attention[cite: 1]. 
...
To capture relative positional dependencies across variable sequence lengths without disrupting vector norms, this engine bypasses traditional absolute or learned position vectors[cite: 1]. Instead, it implements Rotary Positional Embeddings (RoPE)[cite: 1], which apply a structural rotation to the query and key vectors in the complex plane.

Given a two-dimensional vector vector $\mathbf{x} = (x_1, x_2)^T$ at a discrete sequence position $m$, the transformation is executed via a orthogonal rotation matrix $\mathbf{R}_{\Theta, m}^2$:
...
| :--- | :--- | :--- | :--- |
| **Attention Routing** | **Multi-Head Attention (MHA):** Independent Key/Value channels per Query head. | **Grouped-Query Attention (GQA):** Query heads grouped to share underlying KV projections[cite: 1]. | Drops the memory footprints of the active KV-cache by a factor equal to the group ratio, maximizing inference token throughput[cite: 1]. |
| **Position Mapping** | **Absolute Encoding:** Coordinate vectors added linearly to initial token embeddings. | **Rotary Position (RoPE):** Multiplies tensor projections by rotation matrices dynamically[cite: 1]. | Retains explicit relative spatial geometry and scales reliably past standard context length parameters without structural breakdown. |
...
A perplexity score of $K$ indicates that the model was as confused as if it had to choose uniformly at random among $K$ possible tokens at each prediction step. 
...
*   **Weight Mapping and Key Realignment:** Acquired pre-trained parameter weights from open-source repositories[cite: 1]. Because structural parameter key naming conventions differ heavily from first-principles naming structures, a custom remapping script was engineered to parse the state dictionary, map alternative naming paradigms, and load weights without introducing shape mismatches[cite: 1].
*   **Static KV-Cache Tensor Layouts:** Autoregressive generation natively scales at $\mathcal{O}(N)$ time complexity due to the repeated calculation of historic keys and values. To enforce a stable $\mathcal{O}(1)$ step footprint, a static tensor cache was built[cite: 1]. This cache holds sequence shapes up to max context bounds, indexing past steps via dynamic coordinate slices rather than repeating allocation calls.
...
An elegant, from-scratch implementation of the Llama 3 architecture engineered purely in PyTorch. This engine completely bypasses high-level library abstractions to manually construct the foundational mathematical mechanics, multi-dimensional tensor routing, and autoregressive inference pipeline of modern frontier models.
...
* **O(1) Inference Scaling:** Implements a production-grade static KV-cache mechanism to achieve constant-time complexity per token generation.
...
### Grouped-Query Attention (GQA)
To resolve the massive memory bandwidth bottleneck imposed by the Key-Value (KV) cache during autoregressive decoding, this engine implements Grouped-Query Attention. Query heads are partitioned into $G$ sequential groups, sharing a single Key and Value head.

$$\text{Attention}(Q_i, K_g, V_g) = \text{softmax}\left(\frac{Q_i K_g^T}{\sqrt{d_k}}\right) V_g$$

### Rotary Positional Embeddings (RoPE)
To capture relative positional dependencies across variable sequence lengths, this engine implements RoPE, applying an orthogonal rotation matrix $\mathbf{R}_{\Theta, m}^2$ to the query and key vectors in the complex plane.
...
### SwiGLU Activation Function
The feed-forward network blocks utilize the Swish-Gated Linear Unit (SwiGLU) variant to enhance training stability and representation capacity. 
...
| Engine Component | Legacy Paradigm (e.g., GPT-2 Layout) | Modern Paradigm (Llama 3 Layout) | Practical Advantage |
| :--- | :--- | :--- | :--- |
| **Attention Routing** | **MHA:** Independent KV channels per Query head. | **GQA:** Query heads grouped to share KV projections. | Drops active KV-cache memory footprints, maximizing token throughput. |
| **Position Mapping** | **Absolute Encoding:** Coordinate vectors added to tokens. | **RoPE:** Dynamic rotation matrices. | Retains explicit relative spatial geometry past standard context lengths. |
| **Layer Normalization** | **Standard LayerNorm:** Centering operations relying on sequence means. | **RMSNorm:** Normalizes purely on root mean squares. | Drops the mean calculation overhead for up to a 50% computational speedup. |
| **Activation Mechanics** | **Standard FFN:** Sequential linear transformations. | **SwiGLU:** Dual parallel projections combined via gating. | Provides an expressive, smooth gradient flow across deep hidden states. |

---

## 3. Training Dynamics & Convergence

The primary optimization target is the multi-class Cross-Entropy Loss ($\mathcal{L}$), evaluated over the sequence vocabulary distribution, while structural uncertainty is monitored through Perplexity ($\text{PPL} = \exp(\mathcal{L})$).

![Training Convergence](assets/training_convergence.png)

To safeguard against vanishing or exploding gradients, the training infrastructure implements explicit $L_2$ gradient norm tracking, scaling updates dynamically if the unclipped gradient norm $\|\mathbf{g}\|_2$ exceeds a pre-defined threshold.

---

## 4. Engineering Obstacles & Tensor Topology

* **Weight Mapping and Key Realignment:** Engineered a custom remapping script to parse external state dictionaries and map alternative naming paradigms without introducing shape mismatches.
* **Static KV-Cache Layouts:** Built a static tensor cache holding sequence shapes up to max context bounds, indexing past steps via dynamic coordinate slices rather than repeating memory allocations.
* **Dynamic Shape Alignment:** Traced explicit structural transformations strictly across `[Batch, Sequence, Heads, Head_Dim]` parameters to optimize underlying GPU execution paths and prevent silent broadcast errors.